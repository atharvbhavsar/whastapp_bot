...
import { fetchComplaints, fetchComplaint } from '@/lib/civic-api';
...
const CitizenPortal = () => {
  ...
  const [nearbyIssues, setNearbyIssues] = useState<NearbyIssue[]>(mockNearby);
    ...
    <div className="grid-layout grid-cols-sidebar">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div className="flat-panel">
          ...
          <Button variant="secondary" fullWidth style={{ justifyContent: 'center' }} onClick={handleTrack} disabled={isSearching}>
          ...
          {trackedComplaint && (
            <div style={{ marginTop: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem', borderTop: '1px solid var(--border-color)', paddingTop: '1.5rem' }}>
              {(trackedComplaint.complaint_events || []).map((log: any, i: number) => (
              ...
              {(!trackedComplaint.complaint_events || trackedComplaint.complaint_events.length === 0) && (
                <div style={{ color: 'var(--text-muted)', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <AlertCircle size={16} /> No status updates yet.
                </div>
              )}
            </div>
          )}
