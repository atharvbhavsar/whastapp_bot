...
import { fetchComplaints } from '@/lib/civic-api';
    ...
    const loadTasks = async () => {
      try {
        ...
        const mapped = rawData.slice(0, 5).map((c: any) => ({
          ...
          priority: ((c.severity_label || c.urgency_tag || 'Medium') + '').charAt(0).toUpperCase() + ((c.severity_label || c.urgency_tag || 'Medium') + '').slice(1),
          desc: c.description,
        }));
        setTasks(mapped);
      } catch (e) {
          ...
          {isLoading ? (
            ...
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                ...
                <Button variant="ghost" style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--accent)' }}>Optimize</Button>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {tasks.map((task, i) => (
                  <div key={i} onClick={() => { setActiveTask(task); setTaskStatus('assigned'); }} style={{ background: 'white', padding: '1rem', borderRadius: '8px', border: '1px solid var(--border-color)', cursor: 'pointer' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
              <Button variant="ghost" onClick={() => setActiveTask(null)} style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '1rem', textAlign: 'left', padding: '0' }}>← Back</Button>
              <div style={{ background: 'white', padding: '1.5rem', borderRadius: '8px', border: '1px solid var(--border-color)', marginBottom: '1.5rem' }}>
                ...
                <div style={{ height: '150px', background: '#e3dec9', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', textAlign: 'center', padding: '1rem' }}>
                  {activeTask.desc}
                </div>
              </div>
              <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {taskStatus === 'assigned' && <Button variant="primary" fullWidth onClick={() => setTaskStatus('in_progress')} style={{ justifyContent: 'center', padding: '1rem' }}><Clock size={18} /> Mark In Progress</Button>}
                {taskStatus === 'in_progress' && <Button variant="secondary" fullWidth onClick={() => setTaskStatus('photo_uploaded')} style={{ justifyContent: 'center', padding: '1rem', borderColor: 'var(--accent)', color: 'var(--accent)' }}><Camera size={18} /> Take &quot;After&quot; Photo</Button>}
                {taskStatus === 'photo_uploaded' && (<>
                  <div style={{ background: 'var(--success-light)', color: 'var(--success)', padding: '1rem', borderRadius: '8px', textAlign: 'center', fontSize: '0.9rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}><CheckCircle size={16} /> Photo Geo-Verified by AI</div>
                  <Button variant="primary" fullWidth onClick={() => setTaskStatus('resolved')} style={{ justifyContent: 'center', padding: '1rem' }}><Check size={18} /> Confirm Resolution</Button>
                </>)}
                {taskStatus === 'resolved' && (
                  <div style={{ background: 'var(--bg-tertiary)', padding: '2rem 1rem', borderRadius: '8px', textAlign: 'center' }}>
                    <div style={{ width: '50px', height: '50px', background: 'var(--success)', color: 'white', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1rem' }}><Check size={24} /></div>
