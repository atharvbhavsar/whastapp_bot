...
const TENANT_OPTIONS = [
  { label: "Pune Municipal Corporation", id: "11111111-1111-1111-1111-111111111111" },
  { label: "Mumbai Municipal Corporation", id: "22222222-2222-2222-2222-222222222222" },
  { label: "Nagpur Municipal Corporation", id: "33333333-3333-3333-3333-333333333333" },
];
...
export default function Home() {
  ...
  // Use absolute paths pointing to ai-service
  ...
  const fetchComplaints = async () => {
    ...
    try {
      ...
      if (filterLocation) url += `zone=${filterLocation}&`;
  ...
  useEffect(() => {
    // Phase 4 Governance: Trigger SLA Auto-Escalation Engine Lazily
    fetch(`${API}/api/analytics/trigger-slas`)
      .then(res => res.json())
      .then(data => { if(data.escalated_count > 0) console.log(`Auto-escalated ${data.escalated_count} overdue SLAs`); })
      .catch(console.error);

    fetchComplaints();
    
    // Phase 3: Event-Driven Realtime Pub/Sub architecture
    const channel = supabase
      .channel('complaints-updates')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'complaints_master' },
        (payload) => {
          console.log('Realtime Update Received via Pub/Sub:', payload);
          // Trigger a silent background refetch so the user's dashboard is always live!
          fetchComplaints();
        }
      )
      .subscribe();
  ...
  }, [filterStatus, filterCategory, filterLocation, tenantId]);
          ...
          <>
            <div className="bg-white p-4 rounded-lg shadow-sm border mb-6 flex gap-4 flex-wrap">
              ...
              <div>
                ...
                <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="border rounded px-3 py-1.5 text-sm outline-none">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Location (Ward)</label>
                <select value={filterLocation} onChange={(e) => setFilterLocation(e.target.value)} className="border rounded px-3 py-1.5 text-sm outline-none">
                  <option value="">All Wards (City Wide)</option>
                  <option value="zone_north">North Ward</option>
                  <option value="zone_south">South Ward</option>
                  <option value="zone_east">East Ward</option>
                  <option value="zone_west">West Ward</option>
                  <option value="zone_central">Central Ward</option>
                </select>
              </div>
            </div>
